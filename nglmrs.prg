...
set decimals to 4
 SHELL(.F.)
  ...
  PROPERTY Text "Posting Receipts to the GL and CB in progress... ",MDI .F.,;
  sysmenu .f.,;
  MAXIMIZE .F.,;
  MINIMIZE .F.,;
...
PROGREPS.OPEN()
    ...
    set view to "nglmrs.qbe"
PRIVATE EOFfgtran,PYEAR,PPERIOD,PCOY,PCCENTRE,PACTYP,PACCLASS,PACC,psys,pshdate,pshid,pshno,;
...
set order to cash
   ...
   SET ORDER TO MKEY
       ...
       SET ORDER TO TKEY  
      SELECT fgtran
   set filter to empty(gl_post) .AND.;
   ((((doctype = "52"  .or. DOCTYPE = "22" .or. ;
   left(doctype,1) = '6') .and. (system = "SB" .or. system = "DS" .or. system = "GD"));
   .or. (doctype = "15"  .and. system = "SB")) .and. .not. total = 0;
        .and. (pay_method = "Cash" .or. pay_method = "Cheque";
        .or. left(pay_method,6) = "Direct"  .or. pay_method = "Bank Credit" ;
        .or. pay_method = "Debitnote") .OR. (BTYP="C0" .AND. BCLA = "00"))
 ...
 PROCEDURE RTN2
     local ldd,lmm
     ldd = dtos(fgtran->order_date)  && yyyymmdd
...
pccod = fgtran->dtown
      ...
      pacclass = fgtran->btyp 
      pacc = fgtran->bcla+ fgtran->bcod
      psys = fgtran->system
       pshdate = fgtran->order_date
      pdoc = fgtran->doctype
   *   pserial = trim(fgtran->serialno)
   *   if empty(pserial)
      pserial = fgtran->order_no
    *  endif
    pamt = fgtran->total
    IF .NOT. FGTRAN->GROSS_AMT = 0
    select fgcod
     seek pctyp+pccla+pccod
    if found()
      pamt = fgtran->total - fgtran->gross_amt  && debit account
      else
      QUIT
      ENDIF
      ENDIF
  ...
  *    if .not. pamt = 0
     ...
     replace ccentre with pccentre
      ...
      if  fgtran->btyp = "C0" .and. fgtran->bcla = "00" .and. (FGTRAN->BCOD = "01"  .or. FGTRAN->BCOD = "11") && cash/float a/c
         DO NLCASH_RTN
         ELSE
         if fgtran->btyp = "C0" .and. fgtran->bcla = "00"
      ...
      pactype = fgtyp->actype
      pcoy = fgtran->coy
      pccentre = fgtran->div  && profit centre
      ppcentre = fgtran->cen
      pacclass = fgtran->typ
      pacc = fgtran->cla+fgtran->cod
      psys = fgtran->system
      pshdate = fgtran->order_date
      pdoc = fgtran->doctype
   ...
   ** exclude charge
    ...
    *  if  .not. pamt = 0
      ...
      if  fgtran->typ = "C0" .and. fgtran->cla = "00" .and. (fGTRAN->COD = "01"  .or. FGTRAN->COD = "11") && cash/float a/c
         DO NLCASHP_RTN
         ELSE
      if    fgtran->typ = "C0" .and. fgtran->cla = "00"
        do nlbankP_rtn  && bank a/c
       endif
      endif
      ** charge
...
pccod = fgtran->dtown
 select fgtyp
   ...
   **  charge
    ...
    seek pctyp+pccla+pccod
    if found()
      pamt = fgtran->gross_amt  && debit account
      else
      pamt = 0
      endif
     * pqty = 0
      if  .not. pamt = 0 .AND. .NOT. EMPTY(PCCLA) .AND. .NOT. EMPTY(PCCOD) .AND. .NOT. EMPTY(PCTYP)
      select ngltran
      seek pyear+pcoy+pccentre+ppcentre+pactype+pacclass+pacc+DTOS(PSHDATE)+psys+pdoc
      if .not. found()
      append blank
...
Procedure nlcash_rtn
      ...
      replace RECEIPTS with RECEIPTS + fgtran->total - FGTRAN->GROSS_AMT
        IF FGTRAN->TRANTYPE = 'M-MONEY'
         pcashbook = "M"   && MBILE MONEY DEPOSITS
         ELSE
      ...
      replace trans_date with pshdate
       REPLACE INV_DATE WITH FGTRAN->order_date
      REPLACE LPO WITH FGTRAN->TRANTYPE
      REPLACE INV_NO WITH FGTRAN->PAY_METHOD
        endif
       replace total with total + fgtran->total - FGTRAN->GROSS_AMT
...
Procedure nlcashp_rtn
      ...
      replace PAYMENTS with PAYMENTS + fgtran->total - FGTRAN->GROSS_AMT
        IF FGTRAN->TRANTYPE = 'M-MONEY'
         pcashbook = "M"   && MBILE MONEY DEPOSITS
         ELSE
