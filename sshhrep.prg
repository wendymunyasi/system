...
SELECT st15f
SET FILTER TO  shSUM->Shift_date  = shift_date .and.;
shSUM->Shift_no = shift_no .and. shsUM->Shift_id = shift_id 
go top
if .not. eof()
   do sshhrep_rtn1
   endif 
...
CLOSE PRINTER
 close databases
 do sshhrepp.prg with pluser,_pageno
procedure sshhrep_rtn1  
PML = 0
PEL = 0
PVL = 0
PTL = 0
...
EOFst15f = .f.
    ...
    SELECT st15f
SET FILTER TO  shSUM->Shift_date  = shift_date .and.;
shSUM->Shift_no = shift_no .and. shsUM->Shift_id = shift_id .and. typ='00' 
    GO TOP
    if .not. EOF()
         do
         do fgcon_detail_rtn
...
shSUM->Shift_no = shift_no .and. shsUM->Shift_id = shift_id .and. typ='00' 
   ...
   prtcsales = 0
    ...
    eofscashrec = .f.
    select scashrec
set filter to   shSUM->Shift_date  = shift_date .and. shSUM->Shift_no = shift_no .and. shsUM->Shift_id = shift_id
    set order to OFF
...
pcasou picture '9,999,999.99' at 174
if _plineno > PPL-7
do print_page
endif
 ...
 *   ?
    ?  'SHIFT REPORTS FOR: ' AT 43  STYLE 'B','SHIFT DATE: ' AT 65 STYLE 'B', shSUM->SHIFT_DATE AT 80 STYLE 'B',;
 "SHIFT NO./ID : "+SHSUM->shift_no+'/'+shSUM->shift_id+' - '+ psname at 95 STYLE "B"
   ? '========================================================================================' AT 43 STYLE 'B'
 ? "SECTION A. FUEL METER MOVEMENTS"  AT 3 STYLE 'B'
 ? "-------------------------------"  AT 3 STYLE 'B'
 ? 'PUM' AT 3 STYLE 'B', 'MANUAL MTR' AT 07 STYLE 'B', 'MAN LITRS' AT 18 STYLE 'B','ELEC.  METER' AT 29 STYLE 'B',;
 'ELEC LTRS' AT 43 STYLE 'B','VAR(LTRS)' AT 54 STYLE 'B','THROUGPUT' AT 64 STYLE 'B','RTT(LTR)' AT 74 STYLE 'B',;
 'SOLD LTRS' AT 84 STYLE 'B', 'PUM PRICE' AT 95 STYLE 'B', 'SOLD QTY AMT' AT 107 STYLE 'B',;
 'SHS   METER' AT 122 STYLE 'B', 'SHS MTR AMNT' AT 136 STYLE 'B', 'RTT SHS AMT' AT 151 STYLE 'B',;
 'EXPECTED SALES' AT 163 STYLE 'B',  '  VAR(AMT)' AT 178 STYLE 'B'
 ? '<->' AT 3 STYLE 'B', '<-------->' AT 07 STYLE 'B', '<------->' AT 18 STYLE 'B','<---------->' AT 29 STYLE 'B',;
 '<------->' AT 43 STYLE 'B','<------->' AT 54 STYLE 'B','<------->' AT 64 STYLE 'B','<------>' AT 74 STYLE 'B',;
 '<------->' AT 84 STYLE 'B', '<------->' AT 95 STYLE 'B', '<---------->' AT 107 STYLE 'B',;
 '<--------->' AT 122 STYLE 'B', '<---------->' AT 136 STYLE 'B', '<--------->' AT 151 STYLE 'B',;
 '<------------>' AT 163 STYLE 'B',  '<-------->' AT 178 STYLE 'B'
     if _plineno > PPL - 7
do print_page
endif
...
? STO AT 3, CL_MM_QTY PICTURE '999999999.99' AT 06,ROUND((CL_MM_QTY  - OP_MM_QTY),2) PICTURE '99,999.99' AT 18,;
CL_M_Q PICTURE '9999999999.999' AT 28, ROUND((CL_M_Q-OP_M_Q),2) PICTURE '99,999.99' AT 43,;
(ROUND((CL_M_Q-OP_M_Q),2) - ROUND((CL_MM_QTY  - OP_MM_QTY),2))  PICTURE '999,999.99' AT 53, ;
(SOLD_QTY + TEST_QTY) PICTURE '99,999.99' AT 64,;
TEST_QTY PICTURE '99,999.9' AT 74, SOLD_QTY PICTURE '99,999.99' AT 84, ;
SALE_PRICE PICTURE '99,999.99' AT 95, (SOLD_QTY * SALE_PRICE) PICTURE '999,999,999.99' AT 105,;
CL_M_A PICTURE '9999999999.999' AT 120,;
(CL_M_A-OP_M_A-test_qty*sale_price) PICTURE '999,999,999.99' AT 134,(TEST_QTY*SALE_PRICE) PICTURE '999,999,999.99' AT 148,;
EXP_SALES PICTURE '999999,999.99' AT 164,;
((SOLD_QTY * SALE_PRICE) - (CL_M_A-OP_M_A)+(TEST_QTY*SALE_PRICE))*-1 PICTURE '9999999.99' AT 178
...
? 'TOTAL' STYLE 'B' AT 3, PML PICTURE '99,999.99' AT 18 STYLE 'B', PEL PICTURE '99,999.99' AT 43 STYLE 'B',;
PVL PICTURE '999,999.99' AT 53 STYLE 'B',PTL PICTURE '99,999.99' AT 64 STYLE 'B',;
PRL PICTURE '99,999.9' AT 74 STYLE 'B', PSL PICTURE '99,999.99' AT 84 STYLE 'B', ;
PSA PICTURE '999,999,999.99' AT 105 STYLE 'B',PMA PICTURE '999,999,999.99' AT 134 STYLE 'B',;
PRA PICTURE '999999,999.99' AT 148 STYLE 'B',PES PICTURE '999,999,999.99' AT 163 STYLE 'B',;
PVA PICTURE '99999999.99' AT 177 STYLE 'B'
?
if _plineno > PPL-7
    do print_page
    endif
...
? fgcods2->NAME AT 3, STO AT 40, BBF PICTURE '9,999,999.99' AT 48, (ADJS+PURCHASES) PICTURE '9,999,999.99' AT 65,;
