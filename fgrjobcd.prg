...
CLEAR
SET TALK OFF
Set view to "fgrjobcd.qbe"
eoffgjobcad = .f.
EOFfgjobcad = .F.
PRIVATE PLINES,PAMT,plhd,pdoc
if fgjobcad->system = 'PS'
pdoc = "C"
else
pdoc  = 'N'
endif
 ...
 * ?
  *?
  ...
  ? fgcoy->address AT 3, fgcoy->phone at 50
 ?  left(fgcoy->country,3)+": "+FGCOY->PIN AT 3,"VAT: "+FGCOY->VAT AT 25
  * ? "-----------------------------------------------------------------------------------------------" at 1
 * ?
      ? "DUPLICATE JOB CARD/CASH SALE"  AT 35 STYLE 'B'
   *  ?
 ...
 *    ? LEFT(UPPER(fgjobcad->CUSTOMER),20)  AT 3 STYLE "B"
    ? "-----------------------------------------------------------------------------" AT 1 STYLE "B"
  ? "NO." AT 1 STYLE "B", "QTY" AT 12 STYLE "B", "ITEM DESCRIPTION" AT 17 STYLE "B",  "%VAT" AT 44 STYLE "B",;
   " PRICE " AT 57 STYLE "B", " AMOUNT" AT 71 STYLE "B"
 ? "-----------------------------------------------------------------------------" AT 1 STYLE "B"
...
select fgjobcad
   ...
   PAMT = PAMT + TOTAL
   PVAT = PVAT + tax_amt 
   pserved = left(fgoff2->name,21)
   poff = left(fgoff->name,15)
   
   PGROSS = PGROSS+GROSS_AMT
   PPRICE = list_price
    ? LTRIM(STR(PCNT)) PICTURE "99" AT 2, QTY*-1 PICTURE "99999.99" at 5, LEFT(FGCOD->NAME,28) AT 15,;
...
*? 
*
? "Login User:" at 3,pluser at 15, "SUB TOTAL" AT 50, PGROSS PICTURE "9999,999,999.99" AT 63
?   "VAT:" AT 50, PVAT PICTURE "9999,999,999.99" AT 63

? "Served by:" at 3, left(pserved,15) at 15,  "---------------" AT 63 STYLE "B"
