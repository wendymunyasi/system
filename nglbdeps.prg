...
set decimals to 4
 SHELL(.F.)
  ...
  PROPERTY Text "Posting Cash Deposits/Transfers to the GL and CB in progress... ",MDI .F.,;
  sysmenu .f.,;
  MAXIMIZE .F.,;
  MINIMIZE .F.,;
...
pcashbook,pserial,eofnlbankc,eofnlbank
 SELECT NGLYEARS
   SET ORDER TO MKEY
select nlbank
set order to bank
...
set order to cash
   ...
   SET ORDER TO MKEY
       ...
       SET ORDER TO TKEY   
      SELECT fgtran
      set filter to empty(gl_post) .and. (doctype = "56" .OR. doctype = "53" .or. doctype = '69');
      .and. system = "SB" .and. .not.  total = 0 
      GO TOP
      IF .NOT. EOF()
         DO 
            DO RTN2
 ...
 PROCEDURE RTN2
   ...
   *   if empty(pserial)
    ...
    *  endif
      pamt = fgtran->total * - 1  && CREDIT CASH/BANK A/C
      pqty = 0
       SELECT NGLYEARS
      SEEK "1"+PYEAR
      IF .NOT. FOUND()
      ...
      if fgtran->cod = "01" .or. fgtran->cod = "11"
       do nlcash_rtn && CASH DEPOSIT
       else
       do nlBANKt_rtn && BANK TRANSFER
       endif
      endif
...
Procedure nlcash_rtn
      ...
      replace DEPOSITS with DEPOSITS + fgtran->total * -1
        
        if fgtran->doctype = "56" .OR. fgtran->doctype = "69" 
        IF FGTRAN->trantype = 'M-MONEY'
         pcashbook = "H"  && MOBILE WITHDRAWALS
         ELSE
       pcashbook = "D"   && NORMAL DEPOSITS
       ENDIF
       else
       pcashbook = "T"    && TRANSFERS
       ENDIF
      ...
      replace trans_date with pshdate
      REPLACE INV_DATE WITH FGTRAN->RECDATE
      REPLACE LPO WITH FGTRAN->TRANTYPE
      REPLACE INV_NO WITH FGTRAN->PAY_METHOD
        endif
       replace total with total + fgtran->total * -1
...
Procedure nlBANK_rtn
      ...
      replace DEPOSITS with DEPOSITS + fgtran->total
         if fgtran->doctype = "56" .OR. fgtran->doctype = "69" 
        IF FGTRAN->trantype = 'M-MONEY'
         pcashbook = "H"  && MOBILE WITHDRAWALS
         ELSE
      ...
      REPLACE INV_NO WITH FGTRAN->PAY_METHOD
       ...
       replace total with total + fgtran->total
   
       
Procedure nlBANKt_rtn
