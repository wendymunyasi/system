...
set decimals to 4
 SHELL(.F.)
  ...
  PROPERTY Text "Posting Stock Transfers to the GL and CB in progress... ",MDI .F.,;
  sysmenu .f.,;
  MAXIMIZE .F.,;
  MINIMIZE .F.,;
...
set order to cash
   ...
   SET ORDER TO MKEY
       ...
       SET ORDER TO TKEY  
      SELECT fgadasto
   set filter to empty(gl_post)  .and. .NOT. EMPTY(system) .and.  (doctype = '12' .and. .not. (empty(st_coy);
   .or. empty(st_div) .or. empty(st_sto) .or. empty(st_cen)))  .and. .not. (empty(typ);
 ...
 PROCEDURE RTN2
    ...
    pinvno = ''
      ...
      pamt = fgadasto->qty * fgadasto->unit_cost * -1 && NET PURCHASES LESS vat
      pqty = fgadasto->qty * -1
...
pcoy = "1"   && receiving centre -ve qty and -value
      pccentre = fgadasto->st_div && profit centre
      ppcentre = fgadasto->st_cen
      ...
      psys = fgadasto->system
       ...
       pdoc = '13'  
       pserial = fgadasto->order_no
      pamt = fgadasto->qty * fgadasto->unit_cost && PURCHASES LESS vat
      pqty = fgadasto->qty
